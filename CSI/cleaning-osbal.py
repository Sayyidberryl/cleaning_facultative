...
OUTPUT_FILE = os.path.join("output", "csi_output_osbal.xlsx")
...
_POLIS_KEEP_AS_IS_RE = re.compile(r"(^\s*P[12345]\s*/|/\s*P[12345]\s*$|\+\s*P[12345]\s*$)", re.IGNORECASE)
...
INSURED_JUNK_WORDS = frozenset({
    ...
    "AND", "OR", "THE", "OF", "AS",
    "NON FOOD", "DIV", "VARIOUS", "ATTACHMENT", "VAR"
})
...
_BRANCH_KEYWORD_RE = re.compile(r"^(?:(?:TBK|PT\.?|CV\.?)\s+)*(?:KCU|KANWIL|CABANG)\b", re.IGNORECASE)
...
# BUG FIX #1: mekanisme glue vs replace sekarang dinamis (lihat
# _classify_and_reconstruct di bawah) -- konstanta ini menggantikan
# _MAX_SUFFIX_LEN lama sbg ambang "base dianggap lengkap".
...
def _strip_attached_noise(text: str) -> str:
    ...
    text = re.sub(r"\s*-\s*E#?\s*\d+\s*$", "", text, flags=re.IGNORECASE)
    text = text.replace("'", "")
    return text
    ...
    for tok in tokens:
        ...
        if digits_only and current_base is not None and len(tok) < len(current_base):
            ...
            if is_short_suffix and len(tok) <= 4:
                ...
                continue  # 5-7 digit -> kemungkinan nomor sertifikat, bukan polis -> buang
...
def expand_repetition_chain(val, patterns=None) -> list:
    ...
    # BUG FIX #5 / #10: "POLIS1 S/D POLIS2" (dua nomor polis LENGKAP, bukan
    # sertifikat) dan gabungannya dgn segmen polis polos lain via koma --
    # lihat _try_polis_sd_and_plain_segments().
    ...
    # BUG FIX #2: TBA suffix -- tangkap juga "+TBA" (nempel dgn "+"), tidak
    # cuma " TBA" (spasi) seperti versi lama.
    ...
    _various_month_match = re.match(
        r"^\s*(\d{7,})\s*/\s*VARIOUS\s*/\s*(?:MACH\s*,\s*)?"
        r"(?:JANUARI|FEBRUARI|MARET|APRIL|MEI|JUNI|JULI|AGUSTUS|SEPTEMBER|OKTOBER|NOVEMBER|DESEMBER|"
        r"JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER)\b.*$",
        val, flags=re.IGNORECASE,
    )
    if _various_month_match:
        return [_various_month_match.group(1)]

    _various_mach_match = re.match(
        r"^\s*(\d{7,})\s*/\s*VARIOUS\s*/\s*MACH\b.*$",
        val, flags=re.IGNORECASE,
    )
    if _various_mach_match:
        return [_various_mach_match.group(1)]

    _compact_cn_match = re.match(
        r"^\s*-?\s*(\d{3,})\s*/\s*CN\s*/\s*(\d{2,})\s*/\s*(\d{2})\s*/\s*(\d{2})\s*$",
        val,
        flags=re.IGNORECASE,
    )
    if _compact_cn_match:
        return [_compact_cn_match.group(1) + "CN" + "".join(_compact_cn_match.groups()[1:])]
...
def extract_certificate_slots_csi(raw_value, clean_polis_values=None) -> list:
    ...
    if is_multi_record:
        # BUG FIX #11: 2 tahap -- tahap 1 cek tiap record TANPA bukti
        # sibling (cuma sinyal kuat: keyword CERT, pola khusus, kurung,
        # S/D eksplisit, dash/list >=5 digit, angka ber-leading-zero).
        # Kalau ada record yg lolos tahap 1, baru record yg masih kosong
        # dicoba lagi DENGAN izin memakai sinyal lemah (short_range/angka
        # pendek tanpa leading zero), karena sibling-nya sudah membuktikan
        # value ini memang konteks sertifikat.
        strong_certs = [
        ...
        group_has_strong = any(strong_certs)
        if group_has_strong:
            record_certs = [
...
def _clsd_source_is_valid(value, kind: str, fac_value=None) -> bool:
    ...
    if _P1_CANCEL_RE.fullmatch(text):
        if kind == "polis":
            return False
        if kind == "slip":
            fac_text = "" if _is_blank_or_placeholder(fac_value) else str(fac_value).strip()
            if re.fullmatch(r"P1", fac_text, flags=re.IGNORECASE):
                return True
            return False
...
def _clean_selected_source(kind: str, clsd_value, fac_value) -> list:
    use_clsd = _clsd_source_is_valid(clsd_value, kind, fac_value)
    ...
    if (
        use_clsd
        and kind == "slip"
        and not _is_blank_or_placeholder(clsd_value)
    ):
        clsd_text = str(clsd_value).strip()

        _special_clsd_slip = re.fullmatch(
            r"(\d{6}\s*/\s*CN\s*/\s*\d{4}\s*/\s*\d{2}\s*/\s*\d{2})",
            clsd_text,
            flags=re.IGNORECASE,
        )

        if _special_clsd_slip:
            cleaned = [_special_clsd_slip.group(1)]
            note = _clean_settlement_note(fac_value)
            if note:
                cleaned[0] = f"{cleaned[0]} {note}"
            return cleaned
    ...
    patterns = (
        KNOWN_POLIS_PATTERNS
        if kind == "polis"
        else KNOWN_SLIP_PATTERNS
    )

    cleaned = _refine_list(
        cleaner(source),
        patterns
    )
...
def _select_certificate_source(clsdt_polis_value, fac_polis_value) -> list:
    use_clsd = _clsd_source_is_valid(
        clsdt_polis_value,
        "polis",
        fac_polis_value,
    )
    source = clsdt_polis_value if use_clsd else fac_polis_value
    return extract_certificate_slots_csi(source)
...
def process_data(input_file: str, output_file: str) -> None:
    ...
    for _, row in df.iterrows():
        c_polis = _clean_selected_source(
            "polis",
        ...
        cert_slots = _select_certificate_source(
            row.get(CLSDT_POLIS_COL, ""),
            row.get("FAC_POLICY_NO_ORI", ""),
        )
    ...
    for col in df.columns:
        ...
        if col == "FAC_POLICY_NO_ORI":
            # ============================================================
            # POLICY_CLEAN + CERTIFICATE
            # CERTIFICATE_i tepat di kanan POLICY_CLEAN_i.
            # CERTIFICATE_i hanya dibuat jika ada isi di seluruh file.
            # ============================================================
            max_cert_slots = max(
    ...
    _force_text_format(output_file, prefixes=("POLICY_CLEAN_", "SLIP_CLEAN_", "INSURED_CLEAN_", "CERTIFICATE"))
