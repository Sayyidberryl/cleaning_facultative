"""
Cleaning Data 2 - Osbal | Cedant: KB INS (KOOKMIN BEST INS. EX. LIG INS.)

CATATAN VERSI: v5 -- tambahan pengecualian utk CLSDT berupa "kode P +
catatan" (mis. "P1 CANCEL") ketika FAC ternyata lebih detail. v4
sebelumnya fix konsep seleksi sumber POLIS/SLIP (v2/v3 sempat SALAH
memberi prioritas ke FAC utk POLIS ketika FAC "kelihatan seperti nomor
asli" -- sudah dikoreksi). v3 update lanjutan setelah BUG_KB_INS_pt2.md,
v2 rewrite setelah BUG_KB_INS.md.

CATATAN ARSITEKTUR:
- [FIX v4] Prioritas sumber POLIS & SLIP -- flow standar, SAMA untuk
  KEDUANYA (bukan cuma salah satu), lihat select_value_source():
    1. CLSDT jadi patokan utama.
    2. Kalau CLSDT punya format lengkap & tidak kosong -> pakai CLSDT
       (di-clean spt biasa).
    3. Kalau CLSDT kosong -> pakai FAC (induk), di-clean spt biasa.
    4. Kalau CLSDT cuma berisi kode umum berdiri sendiri (TBA, kode P
       spt P1/P2/dst, Various, atau kode umum lain -- bukan informasi
       spesifik) -> pakai FAC (induk), di-clean spt biasa.
    5. Kolom sumber (FAC atau CLSDT) yang dipakai disesuaikan PER BARIS
       sesuai kondisi masing-masing; proses cleansing pakai rules yang
       sama (_clean_value()) siapa pun sumbernya.
  KECUALI (khusus utk case ini saja, dikonfirmasi user): kalau FAC
  (POLIS ATAU SLIP) berisi narasi "PENYELESAIAN SUSPEND ..." DAN CLSDT
  yang bersangkutan berisi nilai digit valid -> gabungkan basis CLSDT
  (sertifikat dibuang) + narasi FAC (kata mata uang IDR/USD/dst
  dibuang). Contoh: fac="PENYELESAIAN SUSPEND AVIATION 2024 IDR",
  clsdt="2010103102400002-000043" -> "2010103102400002 PENYELESAIAN
  SUSPEND AVIATION 2024" (sertifikat "000043" masuk kolom CERTIFICATE
  terpisah). Contoh lain: clsdt="P1 CANCEL" (bukan kode umum BERDIRI
  SENDIRI karena ada kata "CANCEL", jadi valid) + fac="P1" atau
  "P1+P2" -> tetap pakai CLSDT "P1 CANCEL" (sesuai flow standar poin 2,
  BUKAN pengecualian).
- [BARU v5] PENGECUALIAN KEDUA: kalau CLSDT berupa "kode P + catatan"
  (mis. "P1 CANCEL" -- dimulai kode P<n> tapi bukan standalone murni,
  DAN bukan data digit sungguhan) DAN FAC ternyata punya detail digit
  asli (ada angka polis 10+ digit, walau nempel junk spt "+ P2") -> FAC
  yang dipakai (lebih detail), BUKAN CLSDT. Contoh: clsdt="P1 CANCEL",
  fac="2016101092600016 + P2" -> clean = "2016101092600016" (dari FAC,
  "P2" dibuang sbg junk spt biasa). Sebaliknya kalau FAC-nya JUGA cuma
  kode P polos (tanpa digit asli, mis. "P1" atau "P1+P2") -> TETAP pakai
  CLSDT "P1 CANCEL" (lebih informatif drpd kode P kosong) -- lihat
  contoh di pengecualian pertama di atas.
- Mesin polis/slip (_clean_value) [DIROMBAK v2, masih berlaku]:
    * Ambang pengulangan digit ekor dinaikkan dari 8 ke 12 digit (contoh:
      "3010101102500015+24+1052500001" perlu replace hingga 10 digit
      ekor).
    * Nilai "BASIS + '/' atau spasi + blok sertifikat/register"
      (mis. "2010103102300001/ 001 s/d 009, 011", "...089 S/D 099",
      "... - 007 S.D. 016") dikenali SEBELUM tokenisasi biasa -- kalau
      cocok, POLIS_CLEAN cuma basis-nya saja, dan certificate-nya diisi
      lewat build_certificate() yang mendukung format campuran (lihat
      try_extract_cert_block()).
    * Ekor narasi bulan+tahun (mis. "/ DESEMBER 2017") dibuang duluan --
      itu bukan pengulangan digit.
    * Nilai "garbled"/format asing (huruf+angka campur dalam satu token,
      mis. "P0B3-8006F3-2403A-7740") dibiarkan apa adanya.
    * Prefiks "P<n>/" dibuang SEBELUM proses pisah spasi panjang.
    * [pt2] Basis pendek (<13 digit, mis. "301010") diproses via
      lookahead ke token berikutnya sebelum diputuskan "basis final"
      atau "ditahan menunggu pelengkap".
    * [pt2] Hasil rekonstruksi >5 item -> TIDAK dipakai, nilai ASLI
      dibiarkan apa adanya.
    * [pt2] Cert nempel di dalam chain "+" pada polis DUPLIKAT (mis.
      "X + X-000002") -> basis dianggap 1 (dedup), cert diekstrak
      terpisah.
    * [pt2] Whitelist literal "BORD <BULAN> <TAHUN>".
- CERTIFICATE: prioritas dari CLSDT_SERTF_NO (selalu kosong utk KB INS di
  data mentah), fallback pola yang menempel di CLSDT_POLICY_NO, fallback
  terakhir FAC_POLICY_NO. Kolom CERTIFICATE hanya bersumber dari sisi
  POLIS (tidak ada kolom cert terpisah utk SLIP), sesuai desain awal.
- Insured -- SEMUA rule sama dengan Data 1 & Data 3 (lihat docstring
  KB_INS_cleaning_suspend.py bagian "Insured" untuk daftar lengkap:
  merge-balik lokasi, breakdown FACTORY multi/berulang, singkatan dalam
  kurung, dedup singkatan, CJ BIO discard, dsb).

=====================================================================
CATATAN YANG MASIH PERLU DIKONFIRMASI:
1. Kata kunci pengecualian narasi masih cuma "PENYELESAIAN SUSPEND".
   Kalau ada variasi narasi lain (mis. "SUSPENSE 2024" tanpa kata
   "PENYELESAIAN") yang seharusnya kena rule yang sama, mohon dikabari.
2. [KONFIRMASI] Kasus "3010101102500015+24+1052500001" -- hasil skrip
   ("...3010101052500001") beda dari yang tertulis di bug report awal
   ("...30101011052500001", 1 digit lebih panjang). User sudah
   mengonfirmasi perbedaan ini TIDAK MASALAH -- tidak ada perubahan kode.
=====================================================================
"""
...
OUTPUT_FILE = os.path.join("output", "KbIns_output_osbal.xlsx")
...
FULL_LEN_MIN     = 15  # basis di bawah ini dianggap "terpotong" (perlu disambung)
MAX_SHORT_SUFFIX = 12  # token ekor <= ini dianggap pengulangan digit
FRESH_BASE_MIN   = 13  # token >= ini dianggap basis (bukan ekor pendek)
...
def build_certificate(cert_raw) -> str:
    """Lihat docstring versi Data 3 -- rule format identik, dipakai sama
    di ketiga file. Tiap potongan (dipisah koma level-atas) diformat
    sendiri-sendiri; kalau ADA satu saja yang tidak masuk akal -> kosong."""
    if cert_raw is None:
        return ""
...
# ─────────────────────────────────────────────────────────────────────────────
# DETEKSI BLOK SERTIFIKAT/REGISTER YANG NEMPEL DI POLIS
# ─────────────────────────────────────────────────────────────────────────────
...
def resolve_certificate(clsdt_sertf_val, clsdt_polis_val, fac_polis_val) -> str:
    """Prioritas sumber CERTIFICATE: (1) CLSDT_SERTF_NO, (2) pola yang
    menempel di CLSDT_POLICY_NO, (3) fallback ke FAC_POLICY_NO."""
...
# ─────────────────────────────────────────────────────────────────────────────
# MESIN POLIS/SLIP KB INS [DIROMBAK v2]
# ─────────────────────────────────────────────────────────────────────────────
...
# [BARU v2] ekor narasi bulan+tahun (mis. "/ DESEMBER 2017") -- bukan
# pengulangan digit, dibuang duluan.
...
# token yang bukan kode dikenal (TBA/VAR/P<n>/S/D) -> dibiarkan apa adanya.
...
def _clean_value(val) -> list:
    ...
    # [FIX v2] strip prefiks "P<n>/" SEBELUM split spasi panjang, supaya
    # "P1/  <basis>" (2 spasi) tidak salah kepotong jadi ["P1/", "<basis>"].
    ...
    # [BARU v2] deteksi blok sertifikat/register nempel -> polis clean
    # cuma basisnya saja (certificate diurus terpisah lewat resolve_certificate)
    ...
    for i, tok in enumerate(raw_tokens):
        ...
        if len(tok) >= FULL_LEN_MIN:
            ...
            # tahan dulu (jangan tampil dulu); kalau tidak ada token
            # berikutnya, atau berikutnya JUGA basis panjang -> anggap
            # basis ini sudah final apa adanya, langsung tampilkan.
...
# ─────────────────────────────────────────────────────────────────────────────
# [DIROMBAK v4] SELEKSI SUMBER POLIS & SLIP: CLSDT vs FAC
#
# Flow standar (SAMA untuk POLIS dan SLIP -- BUKAN cuma salah satu):
#   1. CLSDT jadi patokan utama.
#   2. Kalau CLSDT punya format lengkap & tidak kosong -> pakai CLSDT,
#      di-clean sesuai rules.
#   3. Kalau CLSDT kosong -> pakai FAC (induk), di-clean sesuai rules.
#   4. Kalau CLSDT cuma berisi kode umum berdiri sendiri (TBA, kode P
#      spt P1/P2/dst, Various, atau kode umum lain -- bukan berupa
#      angka/informasi spesifik) -> pakai FAC (induk), di-clean sesuai
#      rules.
#   5. Kolom sumber (FAC atau CLSDT) yang dipakai disesuaikan PER BARIS
#      sesuai kondisi masing-masing; proses cleansing-nya pakai rules
#      yang sama (_clean_value()) siapa pun sumbernya.
#
# KECUALI (khusus utk case ini saja, dikonfirmasi user): kalau FAC berisi
# narasi "PENYELESAIAN SUSPEND ..." DAN CLSDT berisi nilai digit valid
# (bukan kode umum berdiri sendiri) -> gabungkan basis CLSDT (sertifikat
# dibuang) + narasi FAC (kata mata uang IDR/USD/dst dibuang). Berlaku utk
# POLIS MAUPUN SLIP (fac_polis atau fac_slip yang punya narasi ini).
# ─────────────────────────────────────────────────────────────────────────────

_NARRATIVE_POLIS_RE = re.compile(r"PENYELESAIAN\s+SUSPEND", re.IGNORECASE)
_CURRENCY_WORD_RE = re.compile(r"\b(?:IDR|USD|EUR|GBP|SGD|JPY)\b\.?", re.IGNORECASE)

# [BARU v5] CLSDT berupa "kode P + catatan" (mis. "P1 CANCEL") -- dimulai
# dgn kode P<n> tapi BUKAN standalone murni (ada kata tambahan), dan BUKAN
# data digit sungguhan.
_P_CODE_NOTE_RE = re.compile(r"^\s*P\d+\b", re.IGNORECASE)
...
def _clsd_source_is_valid(value) -> bool:
    if _is_blank_or_placeholder(value):
        return False
    if _STANDALONE_P_RE.fullmatch(str(value).strip()):
        return False
    return True


def _is_p_code_note(value: str) -> bool:
    text = value.strip()
    if not text:
        return False
    return bool(_P_CODE_NOTE_RE.match(text)) and not re.search(r"\d{10,}", text)


def _fac_has_real_detail(fac_str: str) -> bool:
    return bool(re.search(r"\d{10,}", fac_str))


def select_value_source(fac_val, clsdt_val, kind: str) -> list:
    """kind: "polis" atau "slip" -- rule sama, cuma beda pola refine di
    akhir (KNOWN_POLIS_PATTERNS vs KNOWN_SLIP_PATTERNS)."""
    fac_str = "" if pd.isna(fac_val) else str(fac_val).strip()
    clsdt_str = "" if pd.isna(clsdt_val) else str(clsdt_val).strip()
    clsdt_ok = _clsd_source_is_valid(clsdt_str)
    patterns = KNOWN_POLIS_PATTERNS if kind == "polis" else KNOWN_SLIP_PATTERNS

    # pengecualian khusus: FAC narasi "PENYELESAIAN SUSPEND ..." + CLSDT
    # valid -> gabung basis CLSDT + narasi FAC (mata uang dibuang)
    if _NARRATIVE_POLIS_RE.search(fac_str) and clsdt_ok:
        clsdt_clean = _refine_list(_clean_value(clsdt_str), patterns)
        base = clsdt_clean[0] if clsdt_clean else clsdt_str
        narrative = _CURRENCY_WORD_RE.sub("", fac_str).strip()
        narrative = _normalize_spaces(narrative)
        return [_normalize_spaces(f"{base} {narrative}")]

    # [BARU v5] CLSDT cuma "catatan kode P" (mis. "P1 CANCEL") + FAC punya
    # detail digit asli -> FAC menang (lebih detail), meski FAC juga
    # nempel kode P junk (mis. "2016101092600016 + P2"). KECUALI kalau FAC
    # sendiri cuma kode P doang (P1, P1+P2, dst, tanpa digit asli) -> tetap
    # pakai CLSDT "P1 CANCEL" (lebih informatif drpd kode P kosong).
    if _is_p_code_note(clsdt_str) and _fac_has_real_detail(fac_str):
        return _refine_list(_clean_value(fac_str), patterns)

    # flow standar: CLSDT prioritas (kalau valid & bukan kode umum
    # berdiri sendiri), fallback ke FAC
    source = clsdt_str if clsdt_ok else fac_str
    return _refine_list(_clean_value(source), patterns)


# ─────────────────────────────────────────────────────────────────────────────
# CLEAN INSURED [DIROMBAK v2] -- identik dgn KB_INS_cleaning_suspend.py
# ─────────────────────────────────────────────────────────────────────────────
...
def process_data(input_file: str, output_file: str) -> None:
    ...
    for _, row in df.iterrows():
        c_polis = select_value_source(row.get("FAC_POLICY_NO_ORI", ""), row.get(CLSDT_POLIS_COL, ""), "polis")
        c_slip  = select_value_source(row.get("FAC_SLIP_ORI", ""), row.get(CLSDT_SLIP_COL, ""), "slip")
